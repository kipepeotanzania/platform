...
export default function AdminVolunteersPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await apiFetch<VolunteerApp[]>('/admin/volunteer-apps', { token });
      setApps(data);
    } catch (err: any) {
  ...
  const handleDecision = async (approve: boolean, notes?: string) => {
    ...
    try {
      await apiFetch('/admin/decide', {
        method: 'POST',
        body: JSON.stringify({ entityType: 'volunteer', entityId: selected.id, approve, notes }),
        token,
      });
      setSelected(null);
      loadData();
    ...
    <RequireRole roles={['ADMIN']}>
      <div className="space-y-6">
        <header className="space-y-2">
          <h2 className="text-2xl font-semibold">Voluntarios</h2>
          <p className="text-sm" style={{ color: 'var(--brand-muted)' }}>
            Gestiona las solicitudes y actualiza su estado desde este CRM.
          </p>
        </header>
              ...
              <tbody>
                {filtered.map((app) => (
                  <tr key={app.id} className="odd:bg-white/50">
                    ...
                    <td className="px-4 py-3">{app.modality || '—'}</td>
                ...
                {!filtered.length && (
                  <tr>
                    <td className="px-4 py-4 text-center text-[color:var(--brand-muted)]" colSpan={8}>
          ...
          <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-[90]" onClick={() => setSelected(null)}>
            <div className="bg-white rounded-3xl shadow-2xl max-w-lg w-full p-6 space-y-4" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="space-y-1 text-sm">
                <p>
                  <strong>Institución:</strong> {selected.institution || '—'}
                </p>
                <p>
                  <strong>Área:</strong> {selected.area || '—'}
                </p>
                <p>
                  <strong>Modalidad:</strong> {selected.modality || '—'}
                </p>
                <p>
